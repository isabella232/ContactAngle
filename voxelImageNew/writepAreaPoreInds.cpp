/*-------------------------------------------------------------------------*\	
You pAn redistribute this code and/or modify this code under the 
...
int main(int argc, char *argv[])
{
	...
	char* filepA = argv[3];  // file name
	std::ifstream inpA;     ///pA_x.txt
	std::ofstream outpA("outpA.txt");     
	std::cout<<"\nOpenning pA file: "<<filepA<<std::endl;
	inpA.open(filepA);
	assert(inpA);
	...
	for ( unsigned int i=0; i<4;i++)   inpA>>tmpc, std::cout<<" "<<tmpc;
	...
	outpA<<"Number of pores = "<<nPors<<std::endl;	
	for ( ; getline( inpA, line );)
	{
		++countpA;
		inpA >> xx>>yy>>zz>>pA; 
		//~ std::cout<< "\n " <<i<< " " <<j<< " " <<k<<" "<<" "<<pA<<"  \n";
		...
		if(PI>0)
		{
		  outpA << xx<<" "<<yy<<" "<<zz<<" "<<pA<<" "<<PI<<" "<<(PI>=0 ? PRs[PI] : 0) <<" "<<i[PI]<<" "<<j[PI]<<" "<<k[PI]<<"\n"; 
		//~ outpA << xx<<" "<<yy<<" "<<zz<<" "<<" "<<pA<<" "<<PI<<" "<<PRs[PI] <<" "<<i[PI]<<" "<<j[PI]<<" "<<k[PI]<<"\n"; 
		}
...
//~ writepAPoreInds (ImageName)_VElems.mhd contactAngles_x_PI.txt

...
int main(int argc, char *argv[])
{
	...
	outCA<<"Number of pores = "<<nPors<<std::endl;	
	for ( ; getline( inCA, line );)
	{
		...
		if(PI>0)
		{
		  outCA << xx<<" "<<yy<<" "<<zz<<" "<<cosCA<<" "<<CA<<" "<<PI<<" "<<(PI>=0 ? PRs[PI] : 0) <<" "<<i[PI]<<" "<<j[PI]<<" "<<k[PI]<<"\n"; 
		//~ outCA << xx<<" "<<yy<<" "<<zz<<" "<<cosCA<<" "<<CA<<" "<<PI<<" "<<PRs[PI] <<" "<<i[PI]<<" "<<j[PI]<<" "<<k[PI]<<"\n"; 
		}

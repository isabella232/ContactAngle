...
int main(int argc, char *argv[])
{
	...
	std::vector<double> Gt(nThroats,0.0);
	...
	outCAT<<"Number of Pores = "<<nPors<<std::endl;	
	
	for (countTI = 2;countTI<nThroats; ++countTI)
	{
		Gt[countTI] = 1/(4*3.1415926535);
	}

	
	for (countPI = 2;countPI<nPors; ++countPI)
	{
		for (countTI = 2;countTI<nThroats; ++countTI)
		{
			if (PI1[countTI] == countPI || PI2[countTI] == countPI)
			{
				//adf
			}
		}

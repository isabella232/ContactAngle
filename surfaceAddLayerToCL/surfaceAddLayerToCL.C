...
#   include "createTime.H"
	...
	labelList 	pointNewPointsP3(points.size(),-2);
	...
	forAll(edges, ei)
	  if (pMarks[mps[edges[ei].start()]]==4 && pMarks[mps[edges[ei].end()]]==4)
	  {
		++iNewF;
		edge e=edges[ei];
		if (pointNewPointsP3[mps[e.end()]  ]<0) pointNewPointsP3[mps[e.end()]  ]=++iNewP;
		if (pointNewPointsP3[mps[e.start()]]<0) pointNewPointsP3[mps[e.start()]]=++iNewP;
	  }///\\\///\\\///\\\///\\\///
  		Info<<endl<<iNewP*3+3<<" new points,  "<<iNewF*3+3<<" new faces"<<endl;

	///. add 3 faces for each edge in contact line
	faceList  newFaces1_3rd((iNewF+1), face(4));
	iNewF=-1;
	forAll(edges, ei)
	  if (pMarks[mps[edges[ei].start()]]==4 && pMarks[mps[edges[ei].end()]]==4)
	  {
		++iNewF;
		edge e=edges[ei];
		newFaces1_3rd[iNewF][0]=mps[e.end()];///. store two of the vertex indices
		newFaces1_3rd[iNewF][1]=mps[e.start()];///. store two of the vertex indices
		newFaces1_3rd[iNewF][2]=-1;           ///. take care of later
		newFaces1_3rd[iNewF][3]=-1;           ///. take care of later
	  }///\\\///\\\///\\\///\\\///
    ...
    { ///. add newFaces1_3rd to surf123  faces

		faceList facesSorted_All(faces.size()+3*newFaces1_3rd.size());
		...
		for (int i=0; i<faces.size(); ++i) /// water-rock interface
		  ...
		  {
			face f=faces[i];
			forAll(f, fi)	if(pointNewPointsP3[f[fi]]>=0) f[fi] = points.size()+pointNewPointsP3[f[fi]]*3+0; ///. detach and connect to new vertex indices
		...
		for (int i=0; i<newFaces1_3rd.size(); ++i)
		{
			face f=newFaces1_3rd[i];
			
			f[2] = f[1];  /// contact line edge vertices
			f[3] = f[0];  /// contact line edge vertices
			f[1] = points.size()+pointNewPointsP3[f[1]]*3+0;  ///. new vertex index
			f[0] = points.size()+pointNewPointsP3[f[0]]*3+0;  ///. new vertex index
			facesSorted_All[++indF] = f;
		}
		zoneSizes[0]=indF+1;
		///. same as above, zone 2
		for (int i=0; i<faces.size(); ++i) /// water-oil interface
		  if(fMarks[i]==2) 
		  {
			face f=faces[i];
			forAll(f, fi)	if(pointNewPointsP3[f[fi]]>=0) f[fi] = points.size()+pointNewPointsP3[f[fi]]*3+1;
			facesSorted_All[++indF] = f;
		  };
		for (int i=0; i<newFaces1_3rd.size(); ++i)
		{
			face f=newFaces1_3rd[i];
			f[2] = points.size()+pointNewPointsP3[f[1]]*3+1;
			f[3] = points.size()+pointNewPointsP3[f[0]]*3+1;
			facesSorted_All[++indF] = f;
		}
		zoneSizes[1]=indF+1-zoneSizes[0];

		///. same as above, zone 3
		for (int i=0; i<faces.size(); ++i)  /// oil-rock interface
		  if(fMarks[i]==3) 
		  {
			face f=faces[i];
			forAll(f, fi)	if(pointNewPointsP3[f[fi]]>=0) f[fi] = points.size()+pointNewPointsP3[f[fi]]*3+2;
			facesSorted_All[++indF] = f;
		  };
		for (int i=0; i<newFaces1_3rd.size(); ++i)
		{
			face f=newFaces1_3rd[i];
			f[2] = points.size()+pointNewPointsP3[f[1]]*3+2;
			f[3] = points.size()+pointNewPointsP3[f[0]]*3+2;
			facesSorted_All[++indF] = f;
		}
		...
		forAll(points, pi)
		  if(pointNewPointsP3[pi]>=0) 
		  {
			  pointsNew[pointNewPointsP3[pi]*3]=points[pi];
			  pointsNew[pointNewPointsP3[pi]*3+1]=points[pi];
			  pointsNew[pointNewPointsP3[pi]*3+2]=points[pi];
		  }
			  Info <<pointsNew<<endl;

...
int main(int argc, char *argv[])
{
	...
	std::ofstream outRcT("outRcT.txt");     
	...
	outRcT<<"Number of Throats = "<<nThroats<<std::endl;	
	for ( ; getline( inRc, line );)
	{
		...
		if(TI>0)
		{
			outRcT << xx<<" "<<yy<<" "<<zz<<" "<<pKc<<" "<<TI<<" "<<(TI>=0 ? PI1[TI] : 0)<<" "<<(TI>=0 ? PI2[TI] : 0)<<" "<<(TI>=0 ? TRs[TI] : 0) <<" "<<I[TI]<<" "<<J[TI]<<" "<<K[TI]<<" "<<(TI>=0 ? TCs[TI] : 0) <<"\n"; 
		//~ outRcT << xx<<" "<<yy<<" "<<zz<<" "<<pKc<<" "<<PI<<" "<<PRs[PI] <<" "<<i[PI]<<" "<<j[PI]<<" "<<k[PI]<<"\n"; 
		}

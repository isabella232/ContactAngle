...
int main(int argc, char *argv[])
{
	...
	char* fileCA = argv[2];  // file name
	std::ifstream inCA;     ///contactAngles_x.txt
	std::ofstream outCAT("outCACorner.txt");     
	...
	outCAT<<"Contact angles in Coners: "<<std::endl;	
	for ( ; getline( inCA, line );)
	{
		...
		if(TI>0)
		{
		  outCAT << xx<<" "<<yy<<" "<<zz<<" "<<cosCA<<" "<<CA<<" "<<TI<<"\n"; 
		}
		
	}
	std::cout<< "\nTotal Contact angles in corners = " << countCA;

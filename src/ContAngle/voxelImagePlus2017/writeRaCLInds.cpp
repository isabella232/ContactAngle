...
int main(int argc, char *argv[])
{
	...
	std::ifstream inRc;     ///Ra_x.txt
	std::ofstream outRcT("outRa_CL.txt");     
	std::cout<<"\nOpenning Ra_x file: "<<fileRc<<std::endl;
	inRc.open(fileRc);
	assert(inRc);
	...
	outRcT<<"Ra in CL: "<<nThroats<<std::endl;	
	for ( ; getline( inRc, line );)
	{
		...
		//if(TI<=0)	{TI = vimage[kk][jj][ii]; 															///0,0,0
